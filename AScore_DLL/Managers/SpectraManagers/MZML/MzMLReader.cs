    ...
    public sealed class MzMLReader : IDisposable
    {
        ...
        private class IndexList
        {
            public struct IndexPair
            {
                ...
                public IndexPair(string idRef, long offset)
                {
                    Ref = idRef;
                    Offset = offset;
                }
            ...
            private readonly List<IndexPair> _offsets = new List<IndexPair>();
            public List<IndexPair> Offsets { get { return _offsets; } }
            ...
            {
                _offsets.Add(new IndexPair(idRef, offset));
                AddMapForOffset(idRef, offset);
            }

            private void AddMapForOffset(string idRef, long offset)
            {
                if (IndexType == IndexListType.Chromatogram)
                {
                    return;
                }
                long id = Int64.Parse(idRef.Substring(idRef.LastIndexOfAny(new char[] {'=', 'F'}) + 1));
                OffsetsMapNative.Add(idRef, offset);
                if (IndexType == IndexListType.Spectrum)
                {
                    OffsetsMapInt.Add(id, offset);
                    IdNativeMap.Add(id, idRef);
                }
            }
            ...
            public void RegenerateMaps()
            {
                ...
                {
                    AddMapForOffset(offset.Ref, offset.Offset);
                }
            }
        }
        ...
        private readonly List<MS2_Spectrum> _ms2_spectra = new List<MS2_Spectrum>();
        ...
        private MS2_Spectrum ReadSpectrum(XmlReader reader)
        {
            ...
            int scanNum = Int32.Parse(nativeId.Substring(pos + 1));
            ...
            if (is_ms_ms)
            {
                ...
                var pspectrum = new MS2_Spectrum(mzs.Data, intensities.Data, scanNum);
        ...
        private Precursor ReadPrecursor(XmlReader reader)
        {
            ...
            while (reader.ReadState == ReadState.Interactive)
            {
                ...
                switch (reader.Name)
                {
                    ...
                    case "activation":
                        // Schema requirements: one instance of this element
                        reader.Skip(); // "selectedIon" might not have child nodes
                        // We will either consume the EndElement, or the same element that was passed to ReadSpectrum (in case of no child nodes)
                        break;
                    default:
                        reader.Skip();
